...
interface EditCategoryViewProps {
  category: Category;
  onUpdateCategory: (id: number, name: string, type: 'income' | 'expense') => void;
  onDeleteCategory: (id: number) => void;
}

const EditCategoryView: React.FC<EditCategoryViewProps> = ({ 
  category, 
  onUpdateCategory, 
  onDeleteCategory 
}) => {
  ...
  const handleSave = () => {
    if (name.trim()) {
      ...
      onUpdateCategory(category.id, name.trim(), type);
    }
  };

  const handleDelete = () => {
    if (window.confirm(`Are you sure you want to delete the category "${category.name}"? This will also delete all transactions in this category.`)) {
