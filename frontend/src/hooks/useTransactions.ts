...
import { listCategories, createCategory as apiCreateCategory, listTransactions, createTransaction, updateTransaction as apiUpdateTransaction, deleteTransaction as apiDeleteTransaction, getBalance, updateCategory as apiUpdateCategory, deleteCategory as apiDeleteCategory } from '../lib/api';
...
export const useTransactions = (): UseTransactionsReturn => {
  ...
  const updateCategory = useCallback(async (id: number, name: string, type: 'income' | 'expense') => {
    try {
      await apiUpdateCategory(id, { name, type });
      await refresh();
    } catch (err) {
